...
if (-not (Test-Path $pidFile)) {
    try {
        ...
        if ($response.StatusCode -eq 200) {
            throw "agent-service is running, but PID file is missing: $healthUrl"
        }
    }
    catch {
        Write-Output "agent-service is not running"
        exit 0
    }
}
...
Write-Output "agent-service stopped (PID $processId)"
